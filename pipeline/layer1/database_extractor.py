...
# Oracle trigger syntax puts a BEFORE/AFTER/INSTEAD OF <event> clause between
# the trigger name and "ON <table>" — the SQL-Server-shaped pattern above
# never matches it, so this is a separate pattern rather than a tweak to it.
...
# Oracle packages have no equivalent in the SQL-Server-oriented patterns
# above at all — CREATE PROCEDURE/FUNCTION don't cover PACKAGE/PACKAGE BODY.
