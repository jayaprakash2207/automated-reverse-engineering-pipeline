...
class FileFilter:
    ...
    EXCLUDED_DIRS: Set[str] = {
        ...
        # NOTE: deliberately NOT excluding a bare "packages" dir — that name
        # collides with the very common Oracle convention of storing PL/SQL
        # package source under a folder literally named "packages" (e.g.
        # "plsql/packages/"), which would otherwise be silently dropped
        # wholesale. node_modules/vendor/.nuget/bower_components already
        # cover the legitimate vendor-cache cases without that ambiguity.
        "node_modules", "vendor", ".nuget", "bower_components",
        # IDE / tooling
    ...
    EXCLUDED_SUFFIXES: Set[str] = {
        ...
        # Oracle Forms/Reports proprietary binaries — cannot be read as text;
        # export via JDAPI/frmf2xml or "Save As Text" first (see
        # oracle_forms_extractor.py), then the exported .fmt/.mmt/.pld/
        # .frmxml/.mmxml/.pllxml sibling files are what get processed.
        ".fmb", ".mmb", ".pll", ".rdf", ".fmx", ".mmx", ".plx", ".rep",
        # Minified web assets
    ...
    def filter_by_language(self, files: List[str], language: str) -> List[str]:
        ...
        lang_extensions: dict = {
            ...
            "cobol": {".cbl", ".cob", ".cpy"},
            "plsql": {".sql", ".pks", ".pkb", ".pkg", ".trg", ".prc", ".fnc"},
            "oracle_forms": {".fmt", ".mmt", ".pld", ".frmxml", ".mmxml", ".pllxml"},
        }
        exts = lang_extensions.get(language, set())
        matched = [f for f in files if Path(f).suffix.lower() in exts]

        if language == "oracle_forms":
            # Real exports frequently keep the plain ".xml" extension — see
            # language_detector.py / oracle_forms_extractor.py for why this
            # sniffs content instead of requiring a rename.
            from .extractors.oracle_forms_extractor import looks_like_oracle_forms_xml
            matched += [
                f for f in files
                if Path(f).suffix.lower() == ".xml" and looks_like_oracle_forms_xml(f)
            ]
