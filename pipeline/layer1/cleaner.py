    ...
    def _clean_db(self, db: Dict, root_path: str, ts: str) -> Dict:
        ...
        for key, items in db.items():
            ...
            for item in items:
                ...
                # Include "kind" in the dedup key: a package's spec and body
                # legitimately share the same name but are distinct entries
                # (unlike tables/procs/triggers/views, where name alone was
                # always a safe uniqueness key).
                dedup_key = f"{name}|{item.get('kind', '')}"
